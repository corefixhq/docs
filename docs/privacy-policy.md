...
### 1.5 Chrome Extension (Website Recorder)

CoreFix provides an optional Chrome Extension ("Website Recorder") that assists users in recording authenticated web application traffic for authorized security testing.

The extension is designed solely for capturing HTTP/HTTPS traffic (HAR data) from web applications that have been explicitly authorized for testing within the CoreFix platform.

#### Information Collected

When using the extension, we may collect:

* **HAR (HTTP Archive) traffic** generated by the authorized website, including requests and responses.
* **HTTP headers, cookies, session identifiers, authentication tokens, request bodies, response bodies, and other network metadata** required for accurate security testing.
* **Credentials or personal information** that may appear within captured traffic, including usernames, email addresses, passwords, form submissions, API keys, personal information (PII), or other sensitive application data, if such information is transmitted by the website during recording.
* **Workspace (Organization) ID** entered by the user.
* **User email address** used to authenticate recording permissions.
* **Website URL** being recorded.

Because HAR recordings capture network traffic generated by the application, they may contain sensitive information that is transmitted by the website. We therefore recommend performing recordings only against **development, staging, UAT, or other non-production environments** whenever possible.

#### Recording Authorization

The extension includes multiple authorization controls to prevent unauthorized recording.

Recording is permitted only when **all** of the following conditions are satisfied:

* The user authenticates using their CoreFix email address.
* The provided Workspace (Organization) ID matches an existing CoreFix workspace.
* The currently open browser tab exactly matches a website that has already been registered within the CoreFix platform.
* The authenticated user has been explicitly granted permission to record traffic for that project by the project owner, administrator, or collaborator.
* The website owner has confirmed that they are authorized to perform security testing on the target application during project setup.

If any of these conditions are not met, recording cannot begin.

The extension is restricted to the currently authorized website. If the user navigates to another website or switches browser tabs to a different domain, recording is automatically prevented for those websites unless they have been separately authorized through CoreFix.

#### Purpose of Processing

Recorded HAR data is used exclusively to:

* Generate authenticated security scans.
* Reproduce authenticated application flows during vulnerability assessments.
* Execute web application security testing using CoreFix's scanning infrastructure.
* Generate vulnerability findings and security reports.

HAR recordings are **never** used for advertising, profiling, marketing, or training artificial intelligence or machine learning models.

#### Storage and Security

HAR recordings are transmitted securely over encrypted connections (TLS) and stored in encrypted form within Cloudflare R2 object storage.

Access to HAR recordings is limited to:

* Authorized users who have permission to access the associated CoreFix project.
* CoreFix systems responsible for executing requested security scans.

CoreFix personnel access recorded data only when necessary for customer support, troubleshooting, security investigations, or as otherwise required by law.

#### Deletion

Users may delete HAR recordings at any time through the CoreFix platform.

Deleting a project or deleting the associated HAR recording permanently removes the stored recording from CoreFix's systems, subject to any temporary backup retention required for operational reliability.

#### Chrome Extension Permissions

The Chrome Extension requests only the permissions necessary to perform authenticated web application recording and security testing. These permissions are used solely for the functionality described in this Privacy Policy and are not used to monitor general browsing activity.

The extension does not collect browsing history outside of authorized recording sessions and does not record traffic from websites that have not been explicitly authorized within the CoreFix platform.


### 1.6 Report Sharing Data
...
We implement reasonable technical and organizational measures to protect your personal information, including encryption in transit (TLS), encryption of stored credentials and HAR recordings where applicable, access controls, and regular security reviews of our infrastructure. However, no method of electronic transmission or storage is 100% secure, and we cannot guarantee absolute security.
