...
### Viewing Your Plan Limits

Navigate to **Account → Plan & Billing** to see:

- Your current plan tier
- Monthly token allowance and how much you have used
- Build count limit (if applicable)
- Overage rate (cost per 1 000 tokens beyond the allowance)

### Setting Usage Alerts

You can configure email alerts to notify you when usage crosses a threshold:

1. Go to **Account → Usage → Alerts**.
2. Click **Add alert**.
3. Choose a metric: **Monthly Tokens**, **Monthly Cost**, or **Build Count**.
4. Enter a threshold value.
5. Choose whether the alert fires **once** (when the threshold is first crossed) or **repeatedly** (every time a build pushes usage past the threshold).
6. Click **Save alert**.

Alerts are sent to the email address on your account. Multiple team members can each configure their own independent alerts.

<Note>
Usage alerts are informational only — they do not pause or stop scans. Contact your administrator if you need to set a hard spend cap.
</Note>
...
**Why does the Forecasted Cost differ from the LLM Cost?**

LLM Cost is the raw cost of AI API calls. Forecasted Cost adds an estimate for Cloudflare Worker compute time based on pipeline duration and finding count. Hover the **Forecasted Cost** value anywhere it appears to see the formula applied.

**Who can see usage data?**

- **Members** — see usage for projects they are a member of.
- **Administrators** — see usage across all projects in the organization.

**Can I export usage data to my own billing system?**

Yes. Use **Export CSV** on any usage table, or contact support to enable the Usage API, which returns the same data as JSON for programmatic ingestion.

**Why does token count vary between builds on the same project?**

Token count scales with the number and complexity of findings, the amount of source context retrieved for each finding, and any enrichment retries triggered by AI pipeline errors. Builds with many CRITICAL findings typically consume more tokens because the AI generates longer, more detailed remediation guidance.

**How long is usage data retained?**

Usage data is retained for **24 months**. Older data is archived and available on request.
