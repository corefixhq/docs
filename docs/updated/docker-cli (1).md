...
| DAST (auth) | `zap-auth` | Authenticated web scan using credentials or token via OWASP ZAP |
| API fuzzing | `fuzzer` | API fuzzing against OpenAPI/Swagger spec |
| ZAP API fuzzing | `zap-fuzzer` | ZAP-based API fuzzing |
| SSL/TLS | `testssl` | SSL/TLS configuration and certificate analysis via testssl.sh |
